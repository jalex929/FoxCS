...
- Also resolved the doc-lineage question flagged in the entry below, partially: `python-certification-workbook-map.md` and `gmetrix-content-mapping.md` are unambiguously real, Jay-authored, and now the acted-on source. The separate open question about which of the *two candidate skills-map Google Doc links* (this session's vs. 2026-09-04's) is current remains unresolved — different question, not addressed by today's uploads.
