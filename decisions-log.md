...
**Read both in full:**
- **`gmetrix-content-mapping.md` is not a distinct document.** It's a raw, unprocessed export covering Units 16-20 only, and it's byte-for-byte the same content already imported into `skills-map.md`'s own Units 16-20 sections (diffed to confirm). No separate merge was needed from it. Its one distinct piece — a "Certification Domain-to-Unit Review Map" table — matches `python-certification-workbook-map.md`'s own domain-ownership table exactly, so it added no new information once that file was read too.
- **`python-certification-workbook-map.md` is the real thing** — a genuinely new, rigorous, workbook-page-by-workbook-page and video-subtopic-by-video-subtopic ownership map, explicitly self-declared as "the canonical source for LearnKey workbook/video ownership," built around a strict "assign once, every page has exactly one FoxCS home" rule. Cross-checked it against every GMetrix tie-in note already in `course-plan.md`. Found it consistent with nearly everything already written there (independently arrived at compatible conclusions across ~18 units) — with two real, concrete errors in the existing text:
  1. **`323-command.py` (Workbook p. 68, "Use Command-Line Arguments," Objective 3.2.3) was marked "skip it" in Unit 03's note**, reasoned there because Domain 6's own command-line file (`614-command.py`) seemed to cover the same ground. That reasoning was wrong — Objectives 3.2.3 and 6.1.4 are separately tested, and the canonical map explicitly assigns 3.2.3 to **Unit 18.6** (deferred from Unit 03, not dropped), as its own Domain-3 activity kept separate from Unit 18's existing Domain-6 `os`/`sys` work, followed by its own Review 3.2 ("First Half Review," Workbook p. 69). Fixed in Unit 03's and Unit 18's notes and the GMetrix Domain Mapping summary table (Domain 3's row now lists Unit 18; Domain 6's row no longer claims `sys` covers command-line arguments).
  2. **Unit 08's note listed "Data Structures" (Objective 1.2.4) as one of Domain 1 Lesson 4's two topics without flagging that it's actually Unit 09's**, not Unit 08's — Unit 09's own note already said this correctly, so this was an internal inconsistency, not a wrong assignment. The canonical map is explicit ("do not assign Construct Data Structures here [Unit 08] — that belongs to Unit 09") and confirms the workbook lists **no file** for that page, resolving the open question in Unit 08's note about whether `124-structure.py` was the right file (it isn't cited by the workbook). `125-list.py` remains genuinely unresolved either way. Fixed Unit 08's and Unit 09's notes and added Unit 09 to the Domain 1 row of the summary table.
- Also resolved the doc-lineage question flagged in the entry below: `python-certification-workbook-map.md` and `gmetrix-content-mapping.md` are unambiguously real, Jay-authored, and now the acted-on source.
- **Doc-lineage question (the two skills-map Google Doc links) resolved by Jay directly, same session:** it's one doc, not two. "FoxCS Python — Course Unit Skills Map" (imported into `skills-map.md`) is the same Google Doc as the 2026-09-04 entry's "FoxCS Python — Units 01-03 Skills Map" — Jay renamed it after that entry was written, and it grew from a Units-01-03-only doc into the full course-wide version under the new name. No reconciliation between two docs was ever needed.

**Not done:** no further changes to `skills-map.md` itself were needed (its content is lesson-pedagogy, not workbook-page ownership, and nothing in the cross-check touched it). Didn't attempt to reconcile `gmetrix-content-mapping.md`'s misleading filename/commit-message with Jay — flagged in `course-plan.md`'s own intro note instead, worth a quick real conversation rather than a guess about what he intended to name it.

---

## 2026-09-08 — Imported the Python "Course Unit Skills Map" Google Doc into the repo as `courses/python/skills-map.md`; corrected 3 lesson titles in `course-plan.md`; added Unit 02.0 (Certiport/GMetrix account setup) with the real credential-tracking form

**Context:** Jay asked for `course-plan.md` to be updated/merged against the Google Doc "FoxCS Python — Course Unit Skills Map" (the up-to-date skill mapping, per Jay directly), and for a Unit 2 lesson so students can set up their Certiport/GMetrix accounts as a class before certification prep starts. He also referenced two markdown files, `gmetrix-content-mapping.md` and `python-certification-workbook-map.md`, plus a "starter-content" folder for the workbook, saying he'd upload the GMetrix mapping content shortly.

**Checked before acting, not assumed:**
- `gmetrix-content-mapping.md` and `python-certification-workbook-map.md` do not exist anywhere in this repo or in Jay's Drive (searched both). The workbook itself lives at the repo root (`Python_v2_Student_Workbook.pdf`, `Python v2 Support Files/`), not in a `starter-content` folder — there's a `starter context/` folder, but it only holds the HTML5/JavaScript workbooks. This part of the task is blocked on Jay's upload; flagged rather than fabricated.
- The Google Doc (id `1XFegY3yzgYcG1ABDHw1qa7KR3_PaiUqUDO89vFMwDuI`, "FoxCS Python — Course Unit Skills Map") is much larger than its own "Status" note claims. That note says "Units 01-06 developed in detail" — actually verified full lesson-by-lesson detail (Skills, Conceptual Model, Common Misconceptions, Adaptive Question Targets, Coding Exercise, Mastery Check Coverage) runs through **Unit 09**, then a lighter "Skills Review" pass covers Units 10-20, plus a certification-prep thread from Unit 19 on. Left the stale claim in place inside the imported file rather than silently rewrite Jay's own doc-in-progress, but flagged it there.
- **Possible doc-lineage conflict, not resolved:** this 2026-09-04-entry (above) points to a *different* Google Doc ("FoxCS Python — Units 01-03 Skills Map", id `10BO0ORoEG6fxi1s3I9LL0UZ0fw6K0CYwoBVGTfc5bzE`) for the same kind of content. Didn't fetch that one to diff against tonight's — Jay named tonight's doc directly as "the up-to-date" one, so treated it as authoritative, but the two links should be reconciled (one may be stale/superseded) before anyone goes looking for skills content in the wrong doc.
- Compared every unit/lesson title in `course-plan.md` against the Doc line-by-line: only 3 mismatches — **04.4** "Integer vs Float Division" → "Division and Floor Division"; **04.8** "Rounding and Reasonableness" → "Rounding, Floor, Ceiling, and Reasonableness"; **07.4** "Parameters" → "Parameters and Arguments". Everything else already matched exactly. Fixed in `course-plan.md`.

**Decided (architecture call, not asked as a question — reasoning below):**
- The Doc's per-lesson skill/misconception/question-bank detail does **not** belong inlined into `course-plan.md`. That file's own stated purpose is "the master checklist" for authoring status (titles, checkboxes, GMetrix/Game-UX/journal tie-in notes) — it was never meant to hold ~14,500 lines of per-lesson pedagogy. `lesson-schema.md`'s own `skills:` field already establishes that this level of detail belongs in each lesson's canonical record once actually authored. Instead: imported the full Doc content, cleaned up, as a new sibling file, **`courses/python/skills-map.md`**, and added a pointer to it from `course-plan.md`'s intro section. `course-plan.md` itself only gained the 3 title fixes plus that pointer and a note flagging the still-missing GMetrix mapping files.
- The imported Doc's own framework sections (Skill Naming, Adaptive Practice Structure, Question-Bank Planning, Code Dropdowns, Practice/Coding-Exercise/Mastery-Check/Project definitions) restate and extend material already governed by `02-authoring-system/objectives-and-skills-proficiency.md`, `content-authoring-standards.md`, `adaptive-practice-model.md`, and `lesson-schema.md` (the parent `CLAUDE.md`'s "Source of Truth" list). Spot-checked, no direct contradictions found — but **not line-by-line reconciled**. Flagged inside `skills-map.md` itself rather than left implicit.
- **Google Doc export came back double-backslash-escaped** (`\#`, `\*`, `\_`, `\=`, etc.) for roughly two-thirds of the document (Units 01-06 and 10-20, apparently typed/pasted as plain text rather than using native Google Docs heading/bullet styles) but clean for Units 07-09 (apparently authored using real Docs formatting). Unescaped programmatically (strip a backslash before any non-alphanumeric character — the only place markdown ever inserts one) and verified against real code blocks (`score = 10`, `__init__`, etc.) before writing the file. Also converted em dashes to plain hyphens throughout on import, per Jay's standing no-em-dash preference for authored course content.

**Unit 02.0 — Getting Ready for Certification:**
- Jay confirmed the Certiport/GMetrix credential-tracking form is now actually built: **https://forms.gle/fQoEDsn3PGY8Sam17**. Couldn't independently re-verify its exact fields (Google's viewform requires an authenticated session this session couldn't reach) — assumed it matches the question draft already in `02-authoring-system/certiport-gmetrix-account-setup.md` (Jay's own source for it) and flagged that assumption in that file rather than presenting it as verified.
- That doc previously floated this step landing in "Unit 1." Jay's direct ask tonight placed it in **Unit 2** instead — added as **02.0 Getting Ready for Certification**, before 02.1, in `course-plan.md`'s checklist, with real content authored at `content/unit_02_variables_and_data/lesson_02_00_getting_ready_for_certification/01_instruction.html`. Built as a lightweight one-off (styled after Unit 00's onboarding pattern: objectives box, concept card, numbered steps, a credential-reminder callout, a link to the real form) rather than the full 5-module skill-lesson shape 02.1 uses — there's no skill being assessed here, no practice/mastery-check/project module makes sense for "create two accounts and fill out a form." Updated `certiport-gmetrix-account-setup.md` and `courses/python/CLAUDE.md`'s Status section to match.

**Not done, blocked on Jay:**
- The actual cross-check against `gmetrix-content-mapping.md` and `python-certification-workbook-map.md` — pending his upload.
- Reconciling the two candidate skills-map Google Doc links (this entry vs. 2026-09-04's).
- Line-by-line reconciliation of `skills-map.md`'s framework sections against the 4 governing authoring docs.
- Placement/content for the same account-setup step in Web Dev, Software Dev, and Game Design/Unity — Python/Game I only tonight.
...
**Root cause, confirmed in source, not assumed:** `public/mod/resource/lib.php` declares `FEATURE_COMPLETION_TRACKS_VIEWS => true` and its only completion trigger is `set_module_viewed()` — a plain file resource genuinely has no path to report anything finer than "was opened." This applies regardless of whether the page inside is tabbed or flat-file; presentation and delivery mechanism are separate axes, and this gap is entirely the delivery-mechanism axis.

**Options compared:** (A) SCORM packaging — mod_scorm confirmed installed on this instance; strong native fit for per-item telemetry (`cmi.interactions`), but adds a player-chrome layer between the student and the content, and package-versioning has real re-upload gotchas. (B) Full native rebuild into H5P/`mod_lesson`/`mod_quiz` — cleanest long-term, but most of the existing custom component library (drag-to-match, code stepper, categorization, sequencing, hotspot) has no native equivalent; ongoing re-authoring cost per lesson. (C) A small custom local plugin exposing one endpoint that logs telemetry and calls Moodle's own generic completion API — confirmed technically viable by reading `public/lib/completionlib.php`: `completion_info::update_state()` is module-type-agnostic and, when a course module's completion is set to `COMPLETION_TRACKING_MANUAL`, can be called by ordinary logged-in-user code with no special capability (acts exactly like the student ticking Moodle's own manual-completion checkbox, just triggered by our JS instead).

**Decided: Option C.** Best match for the stated priorities — zero change to the student-facing page (same tabbed HTML/component library, nothing new to look at or navigate), reuses the telemetry shape `FoxCSTelemetry.js` already emits instead of adopting a new format, and is the smallest build (~150 lines, reusable across every lesson) rather than a rebuild or a packaging pipeline. Known tradeoff, accepted: this is bespoke to FoxCS — no community tooling, no export path to another LMS, and completion is only as trustworthy as any client-reported signal (true of SCORM's own JS API too, not a new weakness this introduces).

**Built and verified end-to-end in `sandbox-adaptive-demo` (course id 9), not just unit-tested in isolation:**
- `local_foxcstelemetry` plugin (source: `07-infrastructure/local-plugins/foxcstelemetry/`, deployed to `/var/www/moodle/public/local/foxcstelemetry/`) — one table (`local_foxcstelemetry_log`) and one endpoint (`log.php`: `bootstrap` returns a sesskey for a static page with no server-side templating; `log` records one telemetry row and, if `complete=1`, calls `update_state()`).
- A real mod_resource (cmid 235) built via `create-sandbox-completion-prototype.php`, completion set to `COMPLETION_TRACKING_MANUAL`, containing a working tabbed instruction page + one interactive drill.
- Logged in as the real `foxcstest` student account over HTTPS (not as admin, not via direct DB/PHP shortcuts) and drove the actual page: bootstrap → log a drill attempt → log `lesson_complete` with `complete=1`. Confirmed by querying Moodle's own tables afterward, not by trusting the endpoint's response alone: `mdl_course_modules_completion` shows `completionstate=1` for that user/cmid, and `mdl_local_foxcstelemetry_log` holds the real event row.

**Not yet done:** pushing a score/XP into the gradebook (deferred — Jay's stated top priority was save+completion accuracy first; XP/grading was explicitly left to the autograder reading this data, matching the already-decided "autograder scans Moodle directly" priority). No decision yet on which real lesson this rolls out to first, or whether it also gets wired into `mod_lesson` Content pages (the 01.5/01.6 pattern embeds tabbed HTML inside a native Lesson page already, which gets its own coarse completion/grade for free from Moodle — this mechanism would only add value there for telemetry on custom JS embedded *within* a Content page, not for the overall lesson's completion, which already works). See `chat-log.md`'s 2026-09-04 entry and `worklog.md` for next steps.

**Supersedes:** nothing structural yet — `CLAUDE.md`'s Purpose section (H5P Instruction / native Lesson Practice / native Assignment Project / native Quiz Mastery Check) still describes the intended shape for what those specific modules cover; this adds a targeted fix for the completion/telemetry gap in *content* pages (resources, and custom JS embedded in Lesson Content pages) that model doesn't cover.
