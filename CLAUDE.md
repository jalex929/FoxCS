...
| FoxCS: Seminar III | `courses/seminar-iii/` | In progress (updated 2026-08-29) — real content exists for a subset of weeks: `teacher-materials/` has presentation decks for Weeks 2-9, `instructional-content/` and `printable-sheets/` cover Weeks 3/4/5/7/9 unevenly (see `worklog.md`'s 2026-08-29 entries for the exact gap list). Weeks 2, 6, 8 have a deck but nothing else yet; Week 1 has no deck. **Not a CS/certification course** — ACT academic readiness (Math/Reading/English/Data) + academic/life skills + postsecondary planning (College Prep vs. Workforce Readiness pathways), organized around a fixed weekly day-of-week rhythm rather than unit-folders. See `courses/seminar-iii/CLAUDE.md` for the real structural mismatches with the rest of FoxCS's model before authoring anything. |
