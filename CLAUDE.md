...
**Run `02-authoring-system/tools/check_live_publish_readiness.py` before every live/non-sandbox deploy** — it enforces items 1 and 3 as a hard, automated gate (`exit(1)` on any finding). Items 2, 4, and 5 need an actual read-through; the script says so in its own output rather than pretending to cover them.
