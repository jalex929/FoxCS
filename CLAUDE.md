...
Phase: **MVP pivot — folder-based content build.** No lesson content authored yet for any course. The grading pipeline (`05-grader/`) and spreadsheet dashboard (`06-data-and-spreadsheets/`) are placeholders — intentionally not started, since they need real content to grade against first, and are now a co-priority with core content rather than downstream of it (see decision below).

**Moodle is paused, not abandoned** (decided 2026-08-04, see `decisions-log.md`). Reason: get real content and the grading engine built and refined first, on the simplest delivery mechanism available, rather than spending authoring cycles on Moodle H5P/Lesson-activity production before the underlying content and grading loop are proven. Moodle is installed locally (`C:\Users\Jay Fox\server\moodle`, `Start Moodle.exe` / `Stop Moodle.exe`, version **5.3dev (Build: 20260722)** — a dev branch, worth confirming intentional before real student data is involved) and stays available to resume onto once there's real content to build the conceptual layer around.
...
| Decision | Choice | Why |
|---|---|---|
| Moodle role | **Paused 2026-08-04.** Was: conceptual layer + course home (video, H5P, guided practice, vocab, light adaptive support, nav, announcements) — not the branching engine. Resumes once MVP folder content and the grading loop are proven. | See Two-Surface Delivery Model above and `decisions-log.md`, 2026-08-04 |
