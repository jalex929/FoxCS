**UPDATE 2026-09-03 (continued session): all 4 pieces below are now built and verified, plus a 5th (Feedback) added mid-session per Jay's request. See `decisions-log.md`'s 2026-09-03 entry for full detail — most importantly, the Mastery Check section below is STALE: it specs "4 fixed essays," but the real live pattern (confirmed against `mdl_quiz_slots` for 01.1/01.3) is a password + random-draw-of-1-from-10-pool design. 01.4 was built to match the real pattern, not this doc's spec. Read the decisions-log entry before treating anything below as current.**
