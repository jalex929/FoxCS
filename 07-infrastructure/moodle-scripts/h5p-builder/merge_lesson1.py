...
write_column_package(merged_guided, "01.6 -- Guided Practice", "/tmp/h5p-build/lesson-1-guided-merged.h5p")
...
write_column_package(merged_indep, "01.7 -- Independent Practice", "/tmp/h5p-build/lesson-1-independent-merged.h5p")
...
stage = "/tmp/h5p-build/_stage_lesson-1-baseline-merged"
os.makedirs(stage + "/content", exist_ok=True)
with open(stage + "/h5p.json", "w") as f:
    json.dump(h5p_manifest, f)
with open(stage + "/content/content.json", "w") as f:
    json.dump(baseline, f)
with zipfile.ZipFile("/tmp/h5p-build/lesson-1-baseline-merged.h5p", "w", zipfile.ZIP_DEFLATED) as z:
    z.write(stage + "/h5p.json", "h5p.json")
    z.write(stage + "/content/content.json", "content/content.json")
print("Built /tmp/h5p-build/lesson-1-baseline-merged.h5p")
...
write_column_package(merged_refl, "01.13 -- Lesson 1 Reflection", "/tmp/h5p-build/lesson-1-reflection-merged.h5p")
