...
- **New assessment tier, name not decided (added 2026-08-20)** — per Jay, a short set of 4-7 small, independent programs per unit, mirroring the Certiport Python Next Gen exam's own multi-file coding lab format (students switch between ~7 mostly-unrelated files under time pressure; comfort with that structure is itself worth building). Jay's own working phrase was "unit test," but that collides with the real CS content already at Lesson 15.6 ("Introduction to Unit Testing," GMetrix Domain 5, `assert`/`try`/`except`) — needs a different name before this is built into any file convention, schema field, or teacher-materials key. See `02-authoring-system/mvp-unit-folder-structure.md`'s "Assessment Tiers by Program Shape" section for the fuller writeup (mastery check = one cohesive program, this new tier = 4-7 small independent ones, project = one larger program). Not scoped: which units get it, how many per course, file-naming convention, grader integration.
