...
- ~~What does the instructional HTML page template actually look like?~~ Waiting on Jay's forthcoming updated image-style-guide/visual-generation reference before finalizing. **The visual-generation reference landed 2026-08-18** — see `02-authoring-system/instructional-image-guide.md` (semantic color system, 7 template families including the new inline-friendly Micro Diagram). **Still open**: the existing lesson HTML template (established in Unit 0 and Python's Unit 01) doesn't yet embed any real illustrations per this new guide — that's a real next step, not done automatically by the guide existing.
...
- ~~Which Unity certification is the mandatory floor for Game II?~~ **Resolved 2026-08-18**: Programmer required, Artist achievable-not-guaranteed (framed around effective class-time use, not a hard gate). See `courses/game-programming-2/course-plan.md`'s Certification Framing section. Still no Unity student workbook exists to build hands-on exercises from the way GMetrix/JavaScript/HTML5 workbooks do — that part is unresolved.
...
- **Whether Game I's post-certification MakeCode Arcade work is deliberately timed to land in the AP-testing/senior-checkout window** — it's a plausible fit because it's real project work a student can drive independently without needing fresh full-class instruction (corrected 2026-08-18: not because it's "low-stakes" — per Jay, it's meant as a genuine test of applying a year's skills, not filler), but that placement hasn't been confirmed as intentional.
...
- ~~New assessment tier, name not decided~~ — **resolved 2026-09-04**: built and shipped as "Coding Exercise" (native Moodle Assignment module), which doesn't collide with Lesson 15.6's real "Introduction to Unit Testing" content. See `02-authoring-system/mvp-unit-folder-structure.md`'s "Assessment Tiers by Program Shape" section. Cardinality also settled: multiple Coding Exercises per lesson are fine, but only ever one Project. Still not scoped: exact file-naming convention for multiple small `.py` files, and grader integration for the file group.

## Level 2 Pathways / New HTML & CSS Course (new 2026-09-10 — see `decisions-log.md`)

- **Relationship between the new standalone HTML & CSS course and Web Dev's existing HTML/CSS content.** Jay wants a separate, standalone HTML & CSS course scoped to efficiently get students through the Certiport HTML/CSS certification content. `courses/web-dev/course-plan.md` already teaches HTML/CSS as part of its own Unit 01+ pathway curriculum, and `courses/software-dev/CLAUDE.md` documents "HTML + CSS" as Stage 1 of the same Web II → Software Dev certification progression Web Dev already owns. Does the new course replace that portion of Web Dev, run alongside it as an accelerated/test-out lane (similar in spirit to Web Dev's own unbuilt Mixed-Experience acceleration diagnostic — see the Web I/II reconciliation question above), or serve a different population/purpose entirely? Not decided — confirm with Jay before scoping `courses/html-css/` (or wherever it ends up living).
- **Level 2 pathway readiness pass** — Jay wants `web-dev`, `game-programming-2`, `software-dev`, and the new HTML & CSS course all "set up so folks can dive in" before real content mapping/generation starts. Exact meaning of "set up" (does existing `CLAUDE.md` + `course-plan.md` skeleton already satisfy this for the first three, or is something more expected — e.g. `content/` folder scaffolding, cert-objective-to-unit mapping tables) not yet clarified.

## Content Model — Python Coding Exercise Save/Download (new 2026-09-10 — see `decisions-log.md`)

- **Coding Exercise autosave + downloadable code file.** Per Jay, Coding Exercises should save student progress as they go (not just on submit) and should let a student download the actual code file via a real hyperlink, rather than only opening/viewing it in place. Coding Exercise is native `mod_assign` file-upload today with no autosave equivalent to Instruction's `local_foxcstelemetry` mechanism. Mechanism not designed yet (server-side autosave vs. browser-local draft; how a downloadable copy is served/generated). **Explicitly deferred until Jay confirms the current live Python class session has ended** — not to be started as a live change while students are mid-session.
