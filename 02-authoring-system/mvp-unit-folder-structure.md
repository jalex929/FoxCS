...
**Reference implementation, current as of 2026-08-30:** `courses/python/content/unit_01_what_is_programming/lesson_01_04_printing_output/`. Read that folder directly alongside this doc — it's the ground truth this doc describes. **All 6 lessons in Unit 01 now match this pattern** (see `../decisions-log.md`'s 2026-08-30 entry) — the 2026-08-06 note calling the other 5 unrebuilt was stale; most had actually already been brought up to the reference pattern in earlier sessions, and the 2026-08-30 pass closed the remaining real gaps (missing categorization/sequencing drills, a missing project step in 01.1, missing application.py steps, and a repo-wide em-dash cleanup).
...
- **Every resource also names its own type directly on the page** — not just the number. Each page's `<h1>` (or, for `.py` files, its header comment) reads `N.N.N Type: Subtitle` (e.g. `1.4.5 Practice`, `1.4.7a Project Instructions: Status Message Board` — **no dash separator, changed 2026-08-30 per the standing no-em-dash rule**, just a space before Type since the colon already separates Type from Subtitle); `00_table_of_contents.html` shows the same type as a small `.toc-type` chip next to each filename. A bare number without a type label isn't enough — per Jay, "the resource itself should be labeled on the page."
...
Every HTML page in Unit 01 carries a sticky dropdown at the top (`<div class="unit-menu-wrap">`) — `☰ Unit 01: What Is Programming?` toggles a panel listing all 6 lessons, each as its own collapsible `<details class="lesson-entry">` (only the current lesson defaults `open`), with that lesson's own files listed inside as a flat link list, done-chips included. Three levels of nesting: unit toggle → per-lesson collapse → file links. Pure `<details>`/`<summary>`, no JS framework, no `<select>`.

**Not a shared component — copy-pasted inline into all ~47 HTML files.** There's no include/partial mechanism in this folder-native MVP model, so every file carries its own full copy of the menu's HTML+CSS. **This means any change to a lesson's file list (add/remove/rename a file) has to be propagated into every other lesson's copy of the menu too, or the menu drifts and shows stale links for that lesson everywhere except its own files.** This bit hard on 2026-08-30 — five parallel rebuild passes each updated their own lesson's copy correctly but left the other five lessons' copies pointing at old filenames, and one rebuild pass simplified its own copy down to *only* its own lesson, dropping the other 5 entirely. Fixed via a sync script, `02-authoring-system/tools/sync_unit01_nav_menu.py`, that regenerates the menu from a single source-of-truth file list per lesson (hardcoded at the top of the script — update it there first if a lesson's file list changes) and overwrites the block identically in every file. **Any future multi-lesson content pass should budget a final sync step for this menu** (re-run the script after editing the `LESSONS` list to match reality), or build a real shared-include mechanism if the course grows past Unit 01 and this keeps recurring.
...
- The unit-wide nav menu's copy-pasted-per-file architecture (see above) — no real fix, just documented so it isn't rediscovered the hard way again.
