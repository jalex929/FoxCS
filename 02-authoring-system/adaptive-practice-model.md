...
This is deliberately smaller than `objectives-and-skills-proficiency.md`'s own floated 5-per-skill rate (1 Core + 2 Reinforce + 2 Extend) — 3-5 items per node instead. Fewer nodes, each authored well, beats more nodes authored thin — same "a few real variants, not one question repeated" principle already in `content-authoring-standards.md`.
