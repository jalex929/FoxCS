...
- **Code-block multiple-choice options get stacked `<pre>` blocks, never inline code in a button/label.** Added 2026-09-04, per Jay. When a question's options are lines of code (not short text), each option is its own full code block with a real radio input to its left, stacked vertically — not code squeezed inline into a button's text. See component-library #18 for the reference implementation and pattern to copy; the inline-code-in-a-button version used ad hoc in the 2026-09-04 sandbox prototype is what this replaces going forward.
