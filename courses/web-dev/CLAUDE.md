...
**Shared Unit 0**: Web II does not author its own onboarding unit. It shares one with Game II — see `../../00-project-overview/shared-unit-00-onboarding.md`. Web II students see the **Level 2** edition (both Web Dev and Unity pathways shown, no Python). **A Web II student is not defaulted into Web Dev** — she may choose Web Dev or Unity, same open choice a Game II student has. Units 1+ are course-specific and live here as normal; how a Unity-choosing Web II student's Units 1+ actually get delivered isn't resolved yet — see Open Questions below and the shared-unit-00 doc's Open Items.
...
Per the starter-context course map: **HTML5 Application Development is the required target credential**, **JavaScript is strongly encouraged** as a second. Matches Jay's general 2026-08-17 framing (one mandatory cert per pathway, a second encouraged where supported).
