...
Two Certiport IT Specialist credentials, already decided by the source map (not open the way Game II's Unity certification choice is):

1. **HTML5 Application Development — required target credential.** Integrates HTML5, CSS3, JavaScript ES6, responsive layout, forms, graphics, events, application state, data access, testing, and the application lifecycle. Per the source map's "Recommended Certification Timing": take this **after** the major application-development sequence (Units 05, 07-15 roughly, through Unit 20/21) — not right after the HTML/CSS units just because the name says "HTML5."
2. **JavaScript — strongly encouraged second credential.** Deeper fluency with JS syntax, data, functions, decisions, loops, the DOM, events, forms, debugging. Attemptable once a student has finished the JS/DOM/events/arrays/objects/loops/forms sequence (roughly Units 07-15), ahead of the HTML5 App Dev credential's own later timing, for students who show readiness.
...
Per `../../CLAUDE.md`'s Hard Constraints: core/certification-critical content must land before mid-April (AP testing runs mid-to-late April; seniors are typically checked out by mid-May; motivation drops hard once these periods hit). Applied here: **Unit 21's capstone should be at final-presentation stage before mid-April**, with the HTML5 Application Development exam itself (and, for ready students, the JavaScript exam earlier, after Unit 15) scheduled ahead of that window, not crammed into it. **Unlike Game I, no confirmed lower-stakes post-AP-testing activity is identified yet for this course** — Game I has planned MakeCode Arcade work for that stretch (see `../python/course-plan.md`); this course has no equivalent proposed. Flagged as an open item, not invented here. Exact week-by-week pacing against real dates is blocked on the official CPS academic calendar (see `../../open-questions.md`).
...
- **No lower-stakes post-AP-testing activity identified for this course**, unlike Game I's MakeCode Arcade. Worth a deliberate decision once Unit 21's real pacing is scoped against the actual CPS calendar.
