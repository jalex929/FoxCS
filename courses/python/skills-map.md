...
**Doc lineage resolved 2026-09-08, per Jay directly:** this is the *same* Google Doc referenced in `decisions-log.md`'s 2026-09-04 entry as "FoxCS Python — Units 01-03 Skills Map" (id `10BO0ORoEG6fxi1s3I9LL0UZ0fw6K0CYwoBVGTfc5bzE`) — Jay renamed it at some point after that entry was written, and it's grown into the full course-wide version of that same original doc, not a second, competing doc. There is only one live skills-map Google Doc; the two links in this repo point to the same one under its old and new names.
