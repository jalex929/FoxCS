...
**Skills map (added 2026-09-08):** `skills-map.md`, in this same folder, is the per-lesson skill/misconception/adaptive-question-target/mastery-check-coverage specification, imported from Jay's Google Doc "FoxCS Python — Course Unit Skills Map" (that Doc remains the live source of truth going forward; this file is a version-controlled snapshot to keep in sync with it). Check it before authoring any lesson's real content. Three lesson titles below (04.4, 04.8, 07.4) were corrected to match that Doc; every other title was checked against it and already matched exactly.

**External video resources (added 2026-09-08):** `video-resources.md`, in this same folder, tracks third-party instructional videos (YouTube) assigned to specific lessons — currently 02.1 (live) and 02.2-02.5 (assigned, not yet embedded since those lessons aren't authored yet). Check it before authoring any lesson that might want a video.

**Canonical workbook/video mapping (added 2026-09-08):** `python-certification-workbook-map.md`, in this same folder, is Jay's own canonical LearnKey/GMetrix workbook-and-video ownership map — it declares itself the source of truth for "where is every LearnKey video, workbook activity, coding exercise, and certification review completed," separate from this file's own GMetrix summary notes below. Cross-checked against this file 2026-09-08; it's consistent with almost everything already written here, with two real corrections applied (see the Unit 03 and Unit 18 notes below): **`323-command.py` (Workbook p. 68, "Use Command-Line Arguments," Objective 3.2.3) is not skipped** — it's explicitly assigned to Unit 18.6 as its own Domain-3 activity, kept separate from Unit 18's Domain-6 `os`/`sys` work, followed by Workbook p. 69's "Review 3.2." This file previously (incorrectly) said that page could be skipped because Domain 6's own command-line file covered the same ground — that reasoning predates the canonical map and is wrong; fixed below. Also clarified: Unit 08's note below listed "Data Structures" as one of Lesson 4's two topics without flagging that it's assigned to **Unit 09**, not Unit 08 (Unit 09's own note already said this; now Unit 08's note says so too, matching the canonical map's explicit "do not assign Construct Data Structures here" instruction) — and the canonical map confirms the workbook lists **no file** for that page, resolving the open "is `124-structure.py` really the right file" question. `125-list.py` remains unconfirmed either way; still worth checking directly during authoring. `gmetrix-content-mapping.md` (also now uploaded) turned out to be a raw, unprocessed export duplicating `skills-map.md`'s own Unit 16-20 content, not a distinct GMetrix-assignment document — no separate merge needed from it beyond what's already in `skills-map.md`, though its "Certification Domain-to-Unit Review Map" table matches `python-certification-workbook-map.md`'s own domain-ownership table and is folded into the summary table below.
...
**Same rule, one level up — confirmed by Jay 2026-09-04: also applies per lesson, not just per activity.** Even when two GMetrix-tied activities are kept cleanly separate (per the rule above), don't stack too many *different* domains inside one lesson's own sitting — it reads as bouncing around to the student even if each individual activity is internally single-domain. A lesson revisiting the *same* domain across its own activities is fine (that's depth, not hopping); a lesson touching several different domains in one sitting is the thing to avoid. Unit 05's existing "two clearly separate activities (Domain 1 operators, then Domain 2 branching)" split is already a real example of this working correctly — kept as two different *lessons* (05.1-05.2 vs. 05.3-05.6) rather than crammed into one. Use that as the template when a future unit's tie-in note references more than one domain: split by lesson, don't just note "these are separate activities" and leave them stacked in the same sitting.
...
Also the home for Domain 1's Assignment Operator content. Lesson 6, Objective 1.3.1 (`131-assignment.py`, "Assignment Order") and its companion Lesson 8, Objective 1.4.1 (`141-assignment.py`, "Using Assignment Operators"). **Placement resolved 2026-09-09 (Jay): both land at 02.7, not 02.1** — see 02.7's own line below and `python-certification-workbook-map.md`'s Workbook p.23/p.32 entries (p.23 explicitly says 02.7; p.32 said "Unit 02 certification extension/review," folded into 02.7 as the natural companion since nothing else in Unit 02 had started on this content). Assignment is still conceptually anchored in Unit 02 overall (vs. the rest of the operator types in Unit 05), just at 02.7 specifically rather than 02.1.*
...
- [x] 02.0 Getting Ready for Certification (Certiport/GMetrix account setup + VS Code setup) 🔍 **live 2026-09-08, due Tue Sept 8** — not a Variables & Data skill lesson; a one-time orientation step placed here per Jay 2026-09-08 (moved from the earlier "Unit 1" placement floated in `../../02-authoring-system/certiport-gmetrix-account-setup.md`). Students create their real Certiport and GMetrix accounts (same username/password across both, for memorability), submit what they used via the class credential-tracking form (https://forms.gle/fQoEDsn3PGY8Sam17), and set up VS Code (Python extension, first `.py` file). See that doc for the full account-setup rationale.
- [x] 02.1 Variables and Memory 🔍 **live 2026-09-08, due Tue Sept 8** (drafted 2026-09-04 as sandbox prototype, all 5 modules; deployed live 2026-09-08) . `courses/python/content/unit_02_variables_and_data/lesson_02_01_variables_and_memory/`. **Scope confirmed by Jay 2026-09-04: 02.1's "printing a variable with text" skill also teaches `+` string concatenation alongside the comma method already built, with explicit practice contrasting the two — not just commas alone.** `.format()` and f-strings stay out of 02.1, deferred to 03.5/03.4. See this unit's GMetrix note below and Unit 03's note for the full sequencing rationale and workbook grounding. **Adaptive practice currently 1-of-each-tier per skill live; a full 2-of-each-tier draft (12 more questions) is written and ready to wire in at `lesson_02_01_variables_and_memory/teacher-materials/practice_question_bank.md`, per Jay's 2026-09-08 standing target.** **Mastery Check promoted to the real course 2026-09-09** (cmid=269, quizid=12, 4 auto-graded items, grade=10) — see `teacher-materials/mastery_check_key.md`.
- [x] 02.2 Integers — due Wed Sept 9 🔍 **live on the real course 2026-09-09** (Instruction cmid=259, Coding Exercise cmid=264). 5 skill nodes: identifies_integer_type, distinguishes_integer_from_string, recognizes_negative_integer, uses_integer_for_countable_quantity, predicts_basic_integer_arithmetic. `courses/python/content/unit_02_variables_and_data/lesson_02_02_integers/`. Also carries the **Domain 1 Pre-Assessment** callout (attempt it in GMetrix before starting, framed as prep not a graded checkpoint) and a Micro Diagram SVG (`lives = 3`). Coding Exercise: `GMETRIX-112-numbers.py` (Workbook p.10 int half), video+task+backup-link-submit, save-as convention `GMETRIX-112-numbers-completed.py` (hyphen, per the workbook's own instruction). **Mastery Check built and live 2026-09-09** (cmid=270, quizid=13, 4 new synthesis items, grade=10, deliberately excludes `str()`/`int()` conversion since that's 02.6's job) — see `teacher-materials/mastery_check_key.md`. Not yet reviewed by Jay; sandbox copy (cmid=250) still exists, unused.
- [x] 02.3 Floats — due Thu Sept 10 🔍 **live on the real course 2026-09-09** (Instruction cmid=260, Coding Exercise cmid=265). 5 skill nodes: identifies_float_type, distinguishes_int_vs_float, recognizes_float_disguised_as_whole, uses_float_for_decimal_quantity, predicts_mixed_numeric_result. `courses/python/content/unit_02_variables_and_data/lesson_02_03_floats/`. Coding Exercise: `GMETRIX-113-numbers.py` (Workbook p.11 float half). **Mastery Check built and live 2026-09-09** (cmid=271, quizid=14, 4 new synthesis items, grade=10, deliberately excludes `str()`/`int()` conversion since that's 02.6's job) — see `teacher-materials/mastery_check_key.md`. Not yet reviewed by Jay; sandbox copy (cmid=251) still exists, unused.
- [x] 02.4 Strings — due Thu Sept 10 (same day as 02.3, per the skills-map Doc) 🔍 **live on the real course 2026-09-09** (Instruction cmid=261, Coding Exercise cmid=266). 5 skill nodes: identifies_string_type, recognizes_quotes_override_appearance, distinguishes_string_from_variable, recognizes_numeric_looking_string, recognizes_boolean_looking_string. `courses/python/content/unit_02_variables_and_data/lesson_02_04_strings/`. Coding Exercise: `GMETRIX-111-str.py` (Workbook p.10 str half). **Mastery Check built and live 2026-09-09** (cmid=272, quizid=15, 4 new synthesis items, grade=10) — see `teacher-materials/mastery_check_key.md`. Not yet reviewed by Jay; sandbox copy (cmid=252) still exists, unused.
- [x] 02.5 Booleans — due Fri Sept 11 🔍 **live on the real course 2026-09-09** (Instruction cmid=262, Coding Exercise cmid=267). 5 skill nodes: identifies_boolean_type, applies_boolean_casing, distinguishes_boolean_from_string, interprets_true_false_as_program_state, selects_boolean_for_two_state_information. `courses/python/content/unit_02_variables_and_data/lesson_02_05_booleans/`. Coding Exercise: `GMETRIX-114-boolean.py` (Workbook p.11 bool half) -- **starter file's bug (`char_life = true`, lowercase) is left in on purpose**, the exercise's first step is running it, reading the real error, then fixing it, which is what actually teaches `applies_boolean_casing` experientially rather than just stating the rule. **Mastery Check built and live 2026-09-09** (cmid=273, quizid=16, 4 new synthesis items, grade=10) — see `teacher-materials/mastery_check_key.md`. Not yet reviewed by Jay; sandbox copy (cmid=253) still exists, unused.
- [x] **Unit 02 Mixed Data-Type Checkpoint / Workbook p.12 Review 1.1** (`114-analyze.py`) — 🔍 **live on the real course 2026-09-09** (Coding Exercise cmid=268, "Unit 02 Checkpoint: Mixed Data Types," sits after 267/2.5 Coding Exercise in section 3's sequence, right where it belongs ahead of 02.6). Part 1 is the real GMetrix Review 1.1 file (5 `print(type(...))` predictions); Part 2 is a FoxCS-original addition covering `skills-map.md`'s explicit seven-value confusion set (`12`/`12.0`/`"12"`/`True`/`"True"`/`score`/`"score"`). Grade set to 5 (not the usual 10), since the workbook itself estimates this at 5 minutes — lighter than 02.2-02.5's fuller Coding Exercises. `courses/python/content/unit_02_variables_and_data/checkpoint_mixed_data_types/`. **The "Review on 1.1" video subtopic still has not been picked/assigned** — not embedded in this build, flagged not guessed, same as every other video decision this unit. Not yet reviewed by Jay.
- [x] **02.7's workbook pages (p.23 Assignment Operators, p.32 Using Assignment Operators)** — **resolved 2026-09-09 (Jay): both land at 02.7**, since nothing had started on that lesson yet. `131-assignment.py`/`141-assignment.py` (confirmed present in `Python v2 Support Files/Domain 1/Student/`) are the companion exercise files. Not yet built — see 02.7's own line below.
- [x] 02.6 Type Conversion — due Mon Sept 14 (past due at build time, flagged not backfilled) — **Instruction + Coding Exercise built 2026-09-10, repo-only, not yet deployed to any Moodle course.** 8 skills: uses_type_function, interprets_type_output, converts_to_string, converts_to_integer, converts_to_float, predicts_valid_conversion, predicts_invalid_conversion, selects_conversion_for_context. `courses/python/content/unit_02_variables_and_data/lesson_02_06_type_conversion/`. Full 6-step Troubleshooting Routine woven in per skills-map.md, worked example built around the real GMetrix Coding Exercise file (`GMETRIX-121-conversion.py`, Workbook p.12 Data Type Conversion, deliberately non-crashing string-repetition bug). See `teacher-materials/coding_exercise_rubric.md` for the CE grading checklist and `teacher-materials/practice_question_bank.md` for the full question bank (2 real Practice-page gaps closed there: `uses_type_function` and `converts_to_float` had no dedicated live item, now drafted). **Mastery Check not yet built** — per this unit's own note below, 02.6/02.7 should get one alongside their Instruction content, same as 02.1-02.5, not deferred. Not yet reviewed by Jay; not yet deployed live.
- [ ] 02.7 Reading Code with Variables — due Tue Sept 15 — **lesson not built as of 2026-09-10; the Code Stepper component itself is now at full capability** (component-library #14, brought up to spec 2026-09-10 — see `decisions-log.md`'s matching entry and `02-authoring-system/telemetry-and-analytics.md`). Still needed for this lesson: real 02.7-specific trace content (sequential variable reassignment, per skills-map.md's "Code Stepper" section, lines 644-780 — the component's generic fruit-loop/grade-lookup demos are not lesson content) built into an actual Instruction page, not just the demo. Scope this as its own build, not a quick add-on. **Also carries the Assignment Operator content resolved 2026-09-09** — Workbook p.23 (Assignment Order, Objective 1.3.1, `131-assignment.py`) and p.32 (Using Assignment Operators, Objective 1.4.1, `141-assignment.py`) as a companion Fill-in-the-Blanks/Workbook Page piece alongside the Code Stepper content, not blended into it.
- [ ] Unit 02 Adaptive Review / Targeted Reteaching — Wed Sept 16
- [x] ~~Unit 02 Mastery Check (cumulative, all of 02.1-02.7)~~ — **dropped 2026-09-09.** `mastery-check-standards.md` (canonical) explicitly recommends against a separate unit-level check — the Unit Project already is the cross-lesson synthesis check. `skills-map.md`'s "Thu Sept 17" cumulative check was the deviation, not the standard; superseded by per-lesson Mastery Checks (02.1 ✅, 02.2 ✅ live; 02.3-02.5 retrofit needed; 02.6/02.7 build alongside their Instruction content). See `decisions-log.md`'s 2026-09-09 entry.
- [ ] Unit 02 Project: Personal Profile Generator — Sept 18-21 — **not built 2026-09-08**. Carries the cross-lesson synthesis role per `mastery-check-standards.md`, now that the unit-level Mastery Check above is dropped — make sure the project genuinely requires combining skills across 02.1-02.7, not just 02.7's, when this gets built.

**2026-09-08 build session note:** 02.2-02.5 built as Instruction-only modules (Learn + Key Terms + adaptive Practice bundled, matching the standing module structure) — no per-lesson Project/Coding Exercise/Mastery Check, since skills-map.md scopes those to the Unit level for 02.2 onward (unlike 02.1, which got its own Project as the pilot-proof pass). **Per-lesson Feedback activity also skipped for all 4** — **resolved 2026-09-09 (Jay): one combined Feedback activity covering 02.2-02.5 together, built once 02.6 lands**, not a per-lesson retrofit like 01.4-01.6/02.1 got. `lesson_02_02_integers/`'s `.md` and `teacher-materials/practice_question_bank.md` (2-per-tier) are written; **02.3/02.4/02.5 are missing their equivalent `.md` and practice_question_bank.md files** — the live Instruction pages are real and deployed, but the standing per-lesson documentation pattern wasn't kept up for time. Do this before starting 02.6/02.7.
- [ ] 02 Project: Personal Profile Generator — work period Sept 18-21, recommended completion Mon Sept 21

**Unit 02 Lock: Monday, September 28** — one week after the recommended completion date (Sept 21), matching the new standing policy Jay set 2026-09-08: **Unit 02 is the first unit to close and lock a week after its final due date.** All Unit 02 lesson content stays accessible until the lock date; nothing hides progressively lesson-by-lesson. Not yet technically implemented in Moodle (most of the unit doesn't exist yet) — implement via a course-module `availability` end-date restriction (or per-activity `cutoffdate`) once 02.2-02.7/Project are live, not before.

**Where this whole schedule comes from:** every date above (02.1 through the Project) is Jay's own "Recommended Deadline" from `skills-map.md`'s per-lesson sections, surfaced here 2026-09-08 so it's not buried inside that file's prose. 02.0 was added onto 02.1's original date rather than shifting the rest of the calendar, since Jay explicitly named 02.0 due the same day he'd already have expected 02.1 (Sept 8) and separately said "Unit 2 should only span as long as it needs to." **Pacing philosophy, Jay's own words 2026-09-08:** due dates target the *average* student's pace, not the fastest or slowest — students who finish early move ahead into later content, students who fall behind are expected to use class time to catch up rather than getting an extended date by default.

## Unit 03: User Input & Strings
**String-formatting sequencing across Units 02-03, confirmed by Jay 2026-09-04, grounded directly in the workbook (`322-format.py`, p.67, Objective 3.2.2 "Print Formatted Text"):** that page walks the exact same three variables through three methods in order — comma-separated `print()` args, `.format()`, then an f-string — and explicitly quizzes students on which one is "the preferred way of formatting strings beginning with Python 3.6" (answer: f-strings). GMetrix never shows `+` concatenation on that page; it's a FoxCS-original addition ahead of the certification content (same pattern as Unit 04's percentages/rounding). The resulting sequence:
- **02.1** (Unit 02) — first exposure, two methods only: comma-separated `print()` args and `+` concatenation, taught side by side with real practice on *both*, specifically so students can articulate the difference (commas auto-convert non-strings and auto-space; `+` requires everything to already be a string, so joining a number needs `str(...)` first — that's the concrete "why" behind the distinction, not just two syntaxes to memorize).
- **03.3 String Concatenation** — revisits and deepens `+` (multi-piece joins, the `str()` conversion requirement, common `TypeError`), reinforcing what 02.1 opened rather than duplicating it.
- **03.4 F-Strings** — the "norming" lesson: introduces f-strings as the method students should default to going forward, echoing the workbook's own "preferred since 3.6" framing. By this point students have three methods to compare (commas, +, f-strings).
- **03.5 String Format** — `.format()`, GMetrix's own middle method, placed last since the workbook itself treats f-strings as superseding it.

*GMetrix tie-in: Domain 3 only. Lesson 3. Read Input from Console, Objective 3.2.1 (`321-input.py`); Print Formatted Text, Objective 3.2.2 (no starter file, string.format()/f-string methods). Fits 03.1-03.5. Lesson 3 also has a third piece, Use Command-Line Arguments, Objective 3.2.3 (`323-command.py`). **Corrected 2026-09-08 per `python-certification-workbook-map.md` (the canonical workbook/video map): this is not skipped.** It's deliberately deferred, not dropped — assigned to **Unit 18.6** instead, once students have actually learned command-line execution, as its own Domain-3 activity kept separate from Unit 18's Domain-6 `os`/`sys` work. (Earlier text here reasoned that Domain 6's own command-line file made this page redundant; that was wrong — Objective 3.2.3 and Objective 6.1.4 are separately tested and both need real assignment for full certification coverage.) (Indexing, previously noted here, moved entirely to Unit 08 alongside the rest of Domain 1's Objective 1.2 run. Keeps this unit Domain-3-only.)*
...
## Unit 08: Lists
*GMetrix tie-in: Domain 1 only. Lesson 3, Objective 1.2.2. Indexing (`122-indexing.py`) (the other half of Lesson 3, Data Type Conversion/1.2.1, is anchored at Unit 02 instead). Lesson 4. Slicing, Objective 1.2.3 (`123-slicing.py`); Data Structures, Objective 1.2.4 is anchored at **Unit 09 instead**, not here (confirmed 2026-09-08 by `python-certification-workbook-map.md`, which explicitly says "do not assign Construct Data Structures here" — see Unit 09's own note below; that same source confirms the workbook lists no file for this page, resolving the earlier open question about whether `124-structure.py` was the right file — it isn't cited by the workbook). Lesson 5, Objectives 1.2.5-1.2.6. Lists and Their Operations, one combined exercise (`126-list_operations.py`; the folder also has a `125-list.py` that looks related but isn't cited by name in the workbook text and isn't resolved by the canonical map either. Check it directly when authoring), Review 1.2 (`126-analyze.py`). Strong fit, especially 08.3 Accessing Items (indexing) and 08.10 List Slicing.*
...
*GMetrix tie-in: none for hands-on practice. The workbook's Domain 1 glossary defines Dictionary, Set, and Tuple Variable as vocabulary (under the "Data Structures" lesson), but there are no dedicated dict/tuple/set exercise files in `Python v2 Support Files/Domain 1/`. Treat this unit as FoxCS-original; a GMetrix vocabulary callout could be borrowed for terminology but not practice content. Flagged as a genuine content gap, not an oversight. See `open-questions.md`. **Confirmed 2026-09-08 by `python-certification-workbook-map.md`:** this is Domain 1 Lesson 4's "Construct Data Structures" video subtopic, Workbook p. 18, Objective 1.2.4, no workbook-listed file — that source's own words: "Sets do not require a full FoxCS unit merely because they appear on this workbook page." FoxCS's own deeper tuple/dictionary/nested-data instruction here is original, beyond what that one vocabulary page covers.*
...
*GMetrix tie-in: two separate single-domain activities, corrected 2026-09-08 per `python-certification-workbook-map.md` (this unit is no longer single-domain — see below).
(1) Domain 6, Lesson 1, Objective 6.1. Io (6.1.1, workbook text lists N/A but `611-io.py` exists), os (6.1.2, `612-os.py`), os.path (6.1.3, `613-ospath.py`), sys (6.1.4, workbook text lists N/A but the folder has `614-sys.py`, `614-command.py`, `614-command.txt`, `614-log.txt`). Review 6.1 (`614-analyze.py`). Strong fit across this half of the unit.
(2) Domain 3, Lesson 3's deferred third piece: Use Command-Line Arguments, Objective 3.2.3 (`323-command.py`, Workbook p. 68), fits **18.6**, followed by Review 3.2 / "First Half Review" (Workbook p. 69, no starter file, required student-created file `323-items-completed`) — the canonical workbook map's own reasoning: this keeps every LearnKey workbook page/video assigned exactly once, deferred here rather than taught in Unit 03 because Unit 03 comes before students know command-line execution. **Keep these two activities separate** (per the one-domain-per-activity rule at the top of this file) even though both land in Unit 18 — don't blend `323-command.py` into the same sitting as `612-os.py`/`613-ospath.py`/`614-sys.py` just because they're thematically similar.*
...
**Added 2026-08-17, corrected 2026-08-18, not yet scoped into units/lessons.** Per Jay: Game I students are expected to earn one certification (IT Specialist – Python) and are not expected to pursue a second the way Game II/Web II students are (see `../../decisions-log.md`'s 2026-08-17 entry on certification framing). After Unit 20's capstone and certification exam, the course moves into **MakeCode Arcade** for 2D game projects.

**Correction, 2026-08-18: this is not low-stakes filler or a wind-down activity.** Per Jay directly — MakeCode Arcade work here is meant as a **real way to put a full year of game-design and programming skills to the test**, in a faster, more visual prototyping environment, not busywork to fill time. It's well-suited to the AP-testing-through-end-of-year stretch (see pacing constraint above) for a different reason than originally stated: it's project-driven work a student can carry forward largely independently, so it doesn't depend on fresh full-class direct instruction or full attendance the way new core content would — the fit is about delivery independence, not reduced rigor. **Not confirmed as deliberately timed there** — flagged as plausible, not decided (see `../../open-questions.md`). No units/lessons drafted for this stretch yet; scope, length, and grading are all open.

**Also see:** `../game-programming-2/course-plan.md` uses MakeCode Arcade as its own Phase 1 on-ramp (opens that course, rather than closing it) — a possible shared-content opportunity between the two courses, not yet scoped. See a related early-introduction idea for MakeCode Arcade (an avatar-creation kickoff activity, all three courses) in `../../00-project-overview/kickoff-avatar-and-intro-activity.md`.
...
|---|---|---|
| Domain 1. Operations Using Data Types and Operators | Data types (str/int/float/bool), conversion, indexing, slicing, data structures overview, list operations, and six operator types (assignment, comparison, logical, arithmetic, identity, containment) | 00 (install/setup only), 02 (data types + assignment operator), 04 (arithmetic operator), 05 (comparison/logical/identity operators), 08 (indexing/slicing/lists), 09 (data-structures vocabulary only), 10 (containment operator) |
...
| Domain 3. File Input/Output and Console Input/Output | open/close/read/write/append, check existence, delete, with statement, console input, formatted print, command-line arguments | 03 (console I/O only), 16 (file I/O), 18 (command-line arguments, deferred from Unit 03) |
...
| Domain 6. System/Command-Line Operations and Built-in Modules | io, os, os.path, sys, math (incl. isnan/sqrt/isqrt/pi), datetime, random | 11 (random), 12 (math), 17 (datetime), 18 (os/sys) |

**Row corrected 2026-09-08** per `python-certification-workbook-map.md`: Unit 09 added to Domain 1 (data-structures vocabulary, Workbook p. 18 — see Unit 09's own note above) and Unit 18 added to Domain 3 (command-line arguments, deferred from Unit 03 — see Unit 03 and Unit 18's own notes above). Domain 6's row no longer says "incl. command-line arguments" for `sys`, since that page (Objective 3.2.3) is actually Domain 3's, not Domain 6's — Unit 18 still covers both, just via two separate single-domain activities.
